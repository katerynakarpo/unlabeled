...
def create_app():
    ...
    with app.app_context():
        db.create_all()
    from users.users.views import init_view
    init_view(app)
    return app


users_app = create_app()


def main(host='0.0.0.0', port=5002, debug=False, threaded=False):
    users_app.run(host=host, port=port, debug=debug, threaded=threaded)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', type=str, default='5003')
